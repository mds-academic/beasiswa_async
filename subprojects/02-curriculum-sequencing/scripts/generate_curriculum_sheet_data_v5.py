"""
Generate Complete Curriculum Sheet Payload v5
Includes the finalized 18-step Scratch SD curriculum (video-first: 1 slide bridge intro + 17 video tutorial asli Kak Laras),
36-step SMP, 36-step SMA, and 12 Changelog records.
"""
import json
import os
...
def build_payload_v5():
    ...
    def format_bridge_row(no, mod_id, mod_title, sid, bmeta, learning_type, kicker, media_link, konsep):
        ...
        practice_desc = practice.get('description', 'Buka Scratch Editor online dan coba pasang balok Getting Started.')
        project_formatted = f"TUTORIAL EDITOR RESMI SCRATCH 💻\n{practice_desc}\nTugas: Buka scratch.mit.edu/projects/editor/?tutorial=getStarted, rangkai blok 'when green flag clicked' -> 'move 10 steps' -> 'say Hello! for 2 seconds' hingga Scratch Cat bergerak dan menyapa."
        
        # Cheatsheet
        bookmarks = bmeta.get('bookmarks', [])
        bm_labels = [b.get('label', '') for b in bookmarks[:5]]
        cheatsheet = f"Panduan Navigasi Scratch Asli:\n" + "\n".join([f"- {lbl}" for lbl in bm_labels]) + "\n- Gunakan tombol Next/Prev atau panah keyboard untuk navigasi slide."
        
        # Capaian
        comp = bmeta.get('completionCriteria', {})
        capaian = comp.get('description', 'Siswa mengenali 4 area utama Scratch Editor resmi MIT dan berhasil membuat program balok pertama.')
        
        status = "Siap (Slide Standalone & Real Scratch CDN Verified)"
    ...
    # ==================== 1. SD (Upper Primary) - 18 Steps Video-First ====================
    ...
    sd_video_curation = {
        ...
        'up-about-4': {
            ...
            'project': "Animasi Menu Makanan: Rangkai kode 'when this sprite clicked' -> 'repeat 5' -> 'next costume' -> 'wait 0.5 seconds' -> 'start sound [pop]'.",
    ...
    for mod in sd_data['modules']:
        ...
        for step in mod['steps']:
            ...
            if stype == 'slide':
                ...
                konsep = bmeta.get('summary', 'Orientasi konsep fundamental coding blok Scratch dengan antarmuka dan blok resmi MIT.')
    ...
    print(f"Constructed SD rows: {len(sd_rows)} steps (Expected: 18)")

    # ==================== 2. SMP & SMA from Payload v4 ====================
    with open("subprojects/02-curriculum-sequencing/output/curriculum_sheet_payload_v4.json", "r", encoding="utf-8") as f:
        p4 = json.load(f)

    ms_rows = p4['smp']
    hs_rows = p4['sma']
    changelog_rows = list(p4['changelog'])

    # Append 12th changelog entry for SD Video-First 18 Steps & Real Scratch CDN Revamp
    changelog_rows.append([
        12,
        "Restrukturisasi Kurikulum Scratch SD Video-First (18 Step) & Revamp Slide Aset Asli Scratch CDN",
        "materi-sd, ops-result-sd, Code.gs, courseData-upperprimary.json, bridge-sd-00.html, bridge-sd-00.json",
        "Materi SD sebelumnya memuat slide redundant (bridge-sd-01 s.d. 04) yang mengulang isi video Kak Laras, serta slide bridge-sd-00 menggunakan visual pihak ketiga Create & Learn dan blok ilustratif yang tidak mencerminkan antarmuka Scratch resmi.",
        "Menyederhanakan kurikulum menjadi video-first 18 step (1 slide intro fondasi + 17 video tutorial resmi Kak Laras). Slide bridge-sd-00 direvamp total dengan panduan membuka editor Scratch tutorial getStarted, menampilkan tangkapan layar antarmuka asli MIT Media Lab, palette warna blok resmi, dan langkah tutorial yang di-upload ke CDN Ruangguru (rg_cdn_web_2). Kolom evaluasi ops-result-sd disesuaikan menjadi 18 step.",
        "⚠️ REDUNDANT & NON-AUTENTIK (22 Step, Aset Pihak Ketiga)",
        "✅ VIDEO-FIRST & AUTENTIK (18 Step, Aset Asli CDN Ruangguru)",
        "Eksekusi otomatis setupAllLMSSheets() via Apps Script CDP, inspeksi UI LMS 18 tab, verifikasi integritas 3 mirror JSON (SHA-256 matching), dan verifikasi visual Google Sheet."
    ])
    ...
    out_file = "subprojects/02-curriculum-sequencing/output/curriculum_sheet_payload_v5.json"
    with open(out_file, 'w', encoding='utf-8') as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)

    print(f"✅ Payload v5 successfully generated!")
