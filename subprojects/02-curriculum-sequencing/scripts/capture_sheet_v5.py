...
def main():
    ...
    # Reload page to ensure latest state is reflected
    print("Reloading sheet...", flush=True)
    cdp.send('Page.reload')
    time.sleep(6)
    
    tabs_to_capture = [
        ("Changelog & Audit Log", "screenshot_changelog_and_audit_log_v5.png"),
        ("materi-sd", "screenshot_materi-sd_v5.png"),
        ("ops-result-sd", "screenshot_ops-result-sd_v5.png")
    ]
    ...
    for tab_name, shot_fname in tabs_to_capture:
        ...
        js_list_tabs = """
        (() => {
            return Array.from(document.querySelectorAll('.docs-sheet-tab-name')).map(t => t.innerText.trim());
        })()
        """
        tab_list = cdp.eval(js_list_tabs)
        print(f"Available sheet tab names: {tab_list}", flush=True)
    ...
    cdp.close()
    print("\n🎉 ALL v5 SCREENSHOTS CAPTURED SUCCESSFULLY!", flush=True)
